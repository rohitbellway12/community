...
<div class="min-h-[calc(100vh-4rem)] bg-slate-100 flex flex-col"
     x-data="cbtExamApp()"
     x-cloak>

    {{-- TOP EXAM CONTROL BAR --}}
    <div class="bg-[#0b1329] text-white border-b border-slate-800 sticky top-0 z-30 shadow-md">
        <div class="max-w-7xl mx-auto px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between gap-2 sm:gap-4">
            
            {{-- EXAM TITLE & SECTION --}}
            <div class="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-amber-400 text-[#0b1329] font-black flex items-center justify-center text-xs shrink-0 shadow-xs">
                    CBT
                </div>
                <div class="min-w-0 flex-1">
                    <h2 class="text-xs sm:text-sm font-black text-white truncate">{{ $test->title }}</h2>
                    <div class="flex items-center gap-1.5 sm:gap-2 text-[10px] text-slate-300 truncate">
                        <span class="truncate">{{ $test->testLevel->name ?? 'General Assessment' }}</span>
                        <span>•</span>
                        <span class="shrink-0">{{ $test->questions->count() }} Qs</span>
                    </div>
                </div>
            </div>

            {{-- DIGITAL COUNTDOWN TIMER & MOBILE CONTROLS --}}
            <div class="flex items-center gap-1.5 sm:gap-4 shrink-0">
                <div class="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-xl border transition-all duration-300 shadow-inner"
                     :class="timerSeconds <= 120 ? 'bg-rose-950/80 border-rose-500 text-rose-300 animate-pulse' : (timerSeconds <= 300 ? 'bg-amber-950/80 border-amber-500 text-amber-300' : 'bg-slate-800/90 border-slate-700 text-white')">
                ...
                {{-- MOBILE PALETTE QUICK TOGGLE --}}
                <button type="button"
                        @click="mobilePaletteOpen = !mobilePaletteOpen"
                        class="lg:hidden inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-400 font-extrabold text-xs border border-slate-700 transition shrink-0 cursor-pointer"
                        title="Question Palette">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/>
                    </svg>
                    <span class="text-[11px]" x-text="'Q ' + (currentQ + 1) + '/' + questions.length"></span>
                </button>
                ...
                <div class="hidden md:flex items-center gap-2.5 pl-3 border-l border-slate-700">
                    <div class="w-8 h-8 rounded-full bg-slate-700 text-amber-400 font-bold flex items-center justify-center text-xs border border-slate-600">
                        {{ strtoupper(substr(auth()->user()->name ?? 'C', 0, 1)) }}
                    </div>
                    <div class="text-left text-xs">
                        <div class="font-bold text-slate-200 truncate max-w-[120px]">{{ auth()->user()->name }}</div>
        ...
        <div class="w-full bg-slate-800 h-1 overflow-hidden">
            <div class="h-full transition-all duration-1000"
                 ...
                 :class="timerSeconds <= 120 ? 'bg-rose-500' : 'bg-amber-400'"></div>
        </div>
    </div>

    {{-- MAIN CBT EXAM BODY --}}
    <div class="flex-1 max-w-7xl w-full mx-auto p-2.5 sm:p-5 flex flex-col lg:flex-row gap-4 sm:gap-5">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/80 shadow-sm flex-1 flex flex-col overflow-hidden">
                
                {{-- QUESTION HEADER META --}}
                <div class="bg-slate-50/80 border-b border-slate-200/80 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2 sm:gap-3 flex-wrap">
                    <div class="flex items-center gap-1.5 sm:gap-2.5 flex-wrap">
                        <span class="px-2.5 sm:px-3 py-1 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-black bg-[#0b1329] text-white">
                    ...
                    <div>
                        <template x-if="isMarkedForReview(currentQuestion.id)">
                            <span class="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-extrabold bg-amber-100 text-amber-900 border border-amber-300">
                                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-amber-500"></span> Marked for Review
                            </span>
                        </template>
                        <template x-if="!isMarkedForReview(currentQuestion.id) && selectedAnswers[currentQuestion.id]">
                            <span class="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-extrabold bg-emerald-100 text-emerald-900 border border-emerald-300">
                                <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-500"></span> Answered
                            </span>
                        </template>
                        <template x-if="!isMarkedForReview(currentQuestion.id) && !selectedAnswers[currentQuestion.id]">
                            <span class="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-[11px] font-extrabold bg-slate-100 text-slate-700 border border-slate-200">
                ...
                <div class="p-4 sm:p-8 flex-1 overflow-y-auto">
                    ...
                    <div class="mb-5 sm:mb-6">
                        <div class="text-sm sm:text-base font-extrabold text-slate-900 leading-relaxed tracking-tight break-words"
                    ...
                    <div class="space-y-2.5 sm:space-y-3 max-w-2xl">
                        <template x-for="option in (currentQuestion.options || [])" :key="'opt_' + (currentQuestion.id || 0) + '_' + option.id">
                            <label class="flex items-start gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border-2 transition-all cursor-pointer select-none"
                                   :class="selectedAnswers[currentQuestion.id] == option.id ? 'border-amber-400 bg-amber-50/60 shadow-xs ring-2 ring-amber-400/20' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50/60'">
                                <div class="pt-0.5 shrink-0">
                                    <input type="radio"
                                           ...
                                           class="w-4 h-4 text-amber-500 border-slate-300 focus:ring-amber-400 cursor-pointer">
                                </div>
                                <div class="flex-1 text-xs sm:text-sm text-slate-800 leading-relaxed font-medium">
                                    <span class="font-bold text-[#0b1329] mr-1.5" x-text="option.option_label + '.'"></span>
                ...
                {{-- CBT EXAM ACTION BAR (NTA / TCS iON STYLE) --}}
                <div class="bg-slate-50 border-t border-slate-200/80 p-3 sm:px-6 sm:py-4 flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-3">
                    
                    {{-- REVIEW & CLEAR ROW --}}
                    <div class="grid grid-cols-2 sm:flex items-center gap-2 w-full sm:w-auto">
                        <button type="button"
                                @click="toggleMarkForReview(currentQuestion.id)"
                                class="inline-flex items-center justify-center gap-1 px-3 sm:px-4 py-2.5 rounded-xl text-[11px] sm:text-xs font-bold transition border cursor-pointer"
                                :class="isMarkedForReview(currentQuestion.id) ? 'bg-amber-100 text-amber-900 border-amber-300 hover:bg-amber-200' : 'bg-white text-slate-700 border-slate-300 hover:bg-slate-100'">
                        ...
                        <button type="button"
                                ...
                                class="inline-flex items-center justify-center px-3 sm:px-4 py-2.5 rounded-xl text-[11px] sm:text-xs font-bold text-slate-600 bg-white border border-slate-300 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer">
                            Clear Response
                        </button>
                    </div>
                    ...
                    <div class="grid grid-cols-2 sm:flex items-center gap-2 w-full sm:w-auto justify-end">
                        <button type="button"
                                @click="prevQuestion()"
                                :disabled="currentQ === 0"
                                class="inline-flex items-center justify-center px-4 py-2.5 rounded-xl text-xs font-bold text-slate-700 bg-white border border-slate-300 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer">
                            ← Previous
                        </button>

                        <button type="button"
                                @click="saveAndNext()"
                                class="inline-flex items-center justify-center gap-1.5 px-5 sm:px-6 py-2.5 rounded-xl text-xs font-extrabold text-white bg-emerald-600 hover:bg-emerald-700 transition shadow-xs cursor-pointer">
                            <span x-text="currentQ === questions.length - 1 ? 'Save & Review' : 'Save & Next'"></span>
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- MOBILE EXAM BOTTOM BAR (1-TAP PALETTE & SUBMIT) --}}
                <div class="lg:hidden flex items-center gap-2 px-3 py-2.5 bg-slate-100/90 border-t border-slate-200">
                    <button type="button"
                            @click="mobilePaletteOpen = true"
                            class="flex-1 py-2.5 rounded-xl font-extrabold text-xs bg-white text-slate-800 border border-slate-300 shadow-2xs flex items-center justify-center gap-1.5 cursor-pointer">
                    ...
                    <button type="button"
                            ...
                            class="flex-1 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider text-white bg-[#0b1329] hover:bg-slate-900 shadow-xs flex items-center justify-center gap-1.5 cursor-pointer">
        ...
        <div class="hidden lg:flex w-80 shrink-0 flex-col gap-4">
            
            <div class="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/80 shadow-sm p-4 sm:p-5 flex flex-col">
                
                {{-- PALETTE HEADER --}}
                <div class="flex items-center justify-between mb-3 sm:mb-4 pb-3 border-b border-slate-100">
                    <h3 class="text-xs font-black uppercase tracking-wider text-slate-900">Question Palette</h3>
                    <span class="text-[11px] font-bold text-slate-400">Total: <strong class="text-slate-800" x-text="questions.length"></strong></span>
                </div>

                {{-- STATUS LEGEND SUMMARY COUNTS --}}
                <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 gap-2 mb-4">
                    <div class="flex items-center gap-2 bg-emerald-50 border border-emerald-200/60 p-2 rounded-xl">
                        <span class="w-6 h-6 rounded-lg bg-emerald-600 text-white font-black flex items-center justify-center text-xs shrink-0" x-text="countAnswered()"></span>
                        <span class="text-[10px] sm:text-[11px] font-bold text-emerald-950">Answered</span>
                    </div>
                    <div class="flex items-center gap-2 bg-slate-50 border border-slate-200/60 p-2 rounded-xl">
                        ...
                        <span class="text-[10px] sm:text-[11px] font-bold text-slate-700">Not Answered</span>
                    </div>
                    <div class="flex items-center gap-2 bg-amber-50 border border-amber-200/60 p-2 rounded-xl">
                        <span class="w-6 h-6 rounded-lg bg-amber-500 text-white font-black flex items-center justify-center text-xs shrink-0" x-text="countMarked()"></span>
                        <span class="text-[10px] sm:text-[11px] font-bold text-amber-950">Marked</span>
                    </div>
                    <div class="flex items-center gap-2 bg-slate-100 border border-slate-200 p-2 rounded-xl">
                        ...
                        <span class="text-[10px] sm:text-[11px] font-bold text-slate-600">Not Visited</span>
                    </div>
                </div>

                {{-- QUESTION NUMBER TILES --}}
                <div class="mb-5">
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Select Question to Navigate:</div>
                ...
                @if($test->tab_switch_limit > 0)
                    <div class="bg-slate-50 border border-slate-200 p-3 rounded-2xl text-[11px] mb-4">
                        <div class="flex items-center justify-between font-bold text-slate-700 mb-0.5">
                            ...
                            <span :class="tabSwitches >= tabSwitchLimit ? 'text-rose-600' : 'text-slate-900'">
                ...
                <div class="mt-auto pt-3 border-t border-slate-100">
                    <button type="button"
                            @click="openSubmitModal = true"
                            class="w-full py-3.5 rounded-xl sm:rounded-2xl font-black text-xs uppercase tracking-wider text-white bg-[#0b1329] hover:bg-slate-900 transition shadow-md flex items-center justify-center gap-2 cursor-pointer">
                        <svg class="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <span>Submit Examination</span>
                    </button>
                    <p class="text-[10px] text-slate-400 text-center mt-2">All answers auto-saved automatically.</p>
                </div>
    ...
    {{-- MOBILE SLIDE-UP QUESTION PALETTE DRAWER (MODAL ON SMALL SCREENS) --}}
    <div x-show="mobilePaletteOpen"
         class="lg:hidden fixed inset-0 z-40 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-xs"
         x-transition:enter="transition ease-out duration-200"
         x-transition:enter-start="opacity-0 translate-y-full sm:translate-y-0"
         x-transition:enter-end="opacity-100 translate-y-0"
         x-transition:leave="transition ease-in duration-150"
         x-transition:leave-start="opacity-100 translate-y-0"
         x-transition:leave-end="opacity-0 translate-y-full sm:translate-y-0"
         style="display: none;">
        
        <div class="bg-white rounded-t-3xl sm:rounded-3xl max-w-md w-full p-5 shadow-2xl border border-slate-200 max-h-[85vh] overflow-y-auto"
             @click.away="mobilePaletteOpen = false">
            
            <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
                <div>
                    <h3 class="text-xs font-black uppercase tracking-wider text-slate-900">Question Palette</h3>
                    <p class="text-[10px] text-slate-400 mt-0.5">Tap any number to jump to question</p>
                </div>
                <button type="button" @click="mobilePaletteOpen = false" class="text-slate-400 hover:text-slate-700 font-bold p-1 text-base">✕</button>
            </div>
    ...
    <div x-show="openSubmitModal"
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl max-w-md w-full p-5 sm:p-6 shadow-2xl border border-slate-200"
            ...
            <div class="text-center">
                <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-amber-50 text-amber-500 flex items-center justify-center mx-auto mb-3">
            ...
            <div class="bg-slate-50 rounded-xl sm:rounded-2xl p-3.5 sm:p-4 my-4 sm:my-5 border border-slate-200 text-xs space-y-2">
                ...
                <div class="flex justify-between font-bold">
                    ...
                    <span class="text-emerald-700" x-text="countAnswered()"></span>
                </div>
                <div class="flex justify-between font-bold">
                    <span class="text-slate-600">Unanswered Questions:</span>
                    <span class="text-rose-600" x-text="countUnanswered() + countNotVisited()"></span>
                </div>
                <div class="flex justify-between font-bold">
                    <span class="text-amber-700">Marked for Review:</span>
                    <span class="text-amber-700" x-text="countMarked()"></span>
                </div>
            ...
            <p class="text-[10px] sm:text-[11px] text-slate-500 text-center mb-4 sm:mb-5 leading-relaxed">
                Once submitted, you will not be able to modify any responses and your final score will be calculated immediately.
            </p>
    ...
    {{-- TIME-UP AUTO-SUBMISSION SCREEN OVERLAY (CRITICAL REAL CBT BEHAVIOR) --}}
    <div x-show="isTimeUp"
        ...
        <div class="bg-white rounded-2xl sm:rounded-3xl max-w-md w-full p-6 sm:p-8 text-center shadow-2xl border border-slate-200">
            <div class="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl sm:rounded-3xl bg-rose-100 text-rose-600 flex items-center justify-center mx-auto mb-4 animate-bounce">
                <svg class="w-7 h-7 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
            </div>
            ...
            <p class="text-xs text-slate-500 mt-2 leading-relaxed">
                The allocated duration of <strong>{{ $test->duration_minutes }} minutes</strong> has ended. The system is automatically securing and submitting your answers now.
            </p>
    ...
    return {
        ...
        getQuestionTileClass(idx) {
            ...
            if (this.markedForReview[q.id]) {
                return ring + 'bg-amber-100 text-amber-900 border border-amber-400';
            }
            if (this.selectedAnswers[q.id]) {
                return ring + 'bg-emerald-600 text-white shadow-xs';
            }
